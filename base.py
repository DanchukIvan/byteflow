...
from asyncio import Condition, gather, wait, create_task, FIRST_COMPLETED, run, sleep
...
from typing import Any, MutableMapping, Callable, ClassVar, Optional, Dict, get_args, get_origin
    ...
    def bind_args(self, container):
        ...
        for obj in container:
            ...
            for attr, annotation in annotations.items():
                lst = [c for c in container if get_base(c) ==
                       annotation or get_base(c) == get_args(annotation) or get_base(c) in get_args(annotation)]
                if lst:
                    ...
                    setattr(obj, attr, lst) if get_origin(
                        annotation) is not None else setattr(obj, attr, lst[0])
...
class ContextLock:
    ...
    current_obj: Optional[object] = field(default=None)
    ...
    async def __wait(self, inst_base):
        ...
        async with condition:
            await condition.wait_for(lambda: self.current_obj not in self.waiters)
...
class MakedTrigger:
    ...
    async def pending(self):
        ...
        pending = [create_task(cond.pending())
                   for cond in self.conditions.keys()]
        while pending:
            done, pending = await wait(pending, timeout=self.loop_timeout, return_when=FIRST_COMPLETED)
            for task in done:
                if task.exception() is None:
                    key = task.result()
                    if key in self.conditions.keys():
                        callback = self.context_controll(
                            self.conditions[key])
                        pending = list(pending).append(create_task(callback))
                else:
                    print(
    ...
    async def context_controll(self, coro):
        async with ctx_lock as lock:
            await lock.watch(coro.args[0])
            await coro()
...
def get_base(instance_or_class):
    check_set = set([YassActor, YassService])
    if not isclass(instance_or_class):
        bases = [c for c in instance_or_class.__class__.__bases__ if c not in (
            ContextMixin, ABC, ABCMeta, object)]
    else:
        bases = [c for c in instance_or_class.__bases__ if c not in (
            ContextMixin, ABC, ABCMeta, object)]
    for base in bases:
        if base in check_set:
            return instance_or_class.__class__ if not isclass(instance_or_class) else instance_or_class
...
class YassActor(ContextMixin, metaclass=meta.YassFullMeta):
    ...
    def get_annotations(self):
        d = {}
        base_classes = [c for c in self.__class__.mro()
...
if __name__ == "__main__":
    ...
    async def test_lock(url):
        coros = [Two(url=url), ChildTwo(url=url), One(
            url=url), Three(url=url), ChildOne(url=url),  ChildOne(url=url)]
