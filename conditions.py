...
from base import conditions_set, YassAttr
...
def time_string_converter(time_str):
    ...
    if isinstance(time_str, str):
        ...
        if regex.search(r'[0-2]?(?(?<=[01]?)[0-9])(?(?<=2)[0-3])[\.\-:;\s]([\.\-:;\s]?[0-5][0-9]){1,3}(?(?=\1)\1)', time_str):
            if len(time_str) == 4:
                time_str = '0' + time_str
...
class TimeCondition(BaseCondition, condition_name='time_condition'):
    ...
    end_time: time | OneShotRun = field(
        converter=time_string_converter, init=False, default=None, repr=True, eq=True, order=True)
    frequency: float = field(validator=validators.instance_of(
        (float, int)), default=0, repr=True, eq=True, order=True)
