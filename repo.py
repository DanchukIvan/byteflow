...
from typing import IO, Any, Optional, ClassVar, NewType, TypedDict
...
from fsspec import available_protocols, AbstractFileSystem, filesystem
import os.path
...
class NetworkRepo(Repo, repo_type='network'):
    ...
    available_repos: ClassVar[list[str]] = ['dropbox', 'http', 'https', 'gcs', 'gs', 'gdrive', 'sftp', 'ssh', 'ftp', 'hdfs', 'arrow_hdfs', 'webhdfs', 's3', 's3a', 'wandb',
                                            'oci', 'ocilake', 'adl', 'abfs', 'az', 'dask', 'dbfs', 'github', 'git', 'smb', 'jupyter', 'jlab', 'libarchive', 'oss', 'webdav', 'dvc', 'hf', 'box', 'lakefs']
    engine: FileSystem = field(init=False)
    io_tool: str = field(init=False)
    session: Optional[object] = field(default=None)
    ...
    async def flush(self, filepath='', force=False, updating=False):
        ...
        if (buffer.size >= self.max_memory_size) or (force or updating and buffer.size > 0):
            ...
            print(f'Storage data is {storaged_data}')
            storaged_buf = BytesIO()
            if len(storaged_data) > 0:
                storaged_buf.writelines(storaged_data)
                delta = set(storaged_buf).symmetric_difference(buffer_data)
                if delta is not None and delta:
                    storaged_buf.writelines(delta)
            else:
                storaged_buf.writelines(buffer_data)
            if force:
            ...
            storaged_buf.seek(0)
            await self.engine._pipe_file(filepath, storaged_buf.getvalue())
            self.opened_fd[filepath] = MemoryFile()
            return
...
TableMetadata = TypedDict(
    'TableMetadata', {'last_commit': Optional[datetime], 'size': int})
TableMapping = TypedDict('TableMapping', {"name": str, "meta": TableMetadata})
...
class SqlRepo(Repo, repo_type='sql'):
    ...
    def __build_engine(self):
        engine = create_engine(self.connection_string,
                               connect_args=self.engine_kwargs, **self.option_kwargs)
        self.engine = engine
        self.engine
    ...
    def flush(self, tablename='', force=False, updating=False):
        ...
        if table_size >= self.max_memory_size or (force or updating and table_size > 0):
            with self.engine.connect() as connection:
