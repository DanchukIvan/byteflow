...
from repo import Repo, NetworkRepo, LocalRepo, SqlRepo
...
class Scraper(ABC, base.YassActor):
    ...
    def make_path(self, storage):
        if (strg_cls := storage.__class__) in (NetworkRepo, LocalRepo):
            ...
            query_name, schema_name = self.resource.query_name, self.resource.schema_name
    ...
    async def scrape(self):
        async with self.storage as repo:
            async for url in self.resource.get_url_for():
                print(url)
                raw_data = await self.client.get(url, headers=self.extra_headers)
